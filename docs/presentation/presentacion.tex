...
\begin{frame}{Nota sobre la disponibilidad de código}
  \begin{alertblock}{El artículo no publica código}
    Zhao et al. (2025) no incluye un repositorio de código ni un enlace a
    implementación oficial --- verificado en el PDF/HTML del artículo y en su
    página de arXiv.
  \end{alertblock}
  \begin{itemize}
    \item No es posible clonar y ejecutar el código del artículo, como sí lo es
      para otros trabajos de la lista de este curso.
    \item Por eso, \textbf{todo lo que sigue es una implementación propia} del
      pipeline de tres etapas, construida directamente a partir de la
      descripción metodológica y las fórmulas del artículo (Ec.~1--7), no a
      partir de un código fuente existente.
    \item Las desviaciones frente al método original quedan documentadas de
      forma explícita en su propia sección, para que la implementación propia
      sea auditable frente al texto fuente.
  \end{itemize}
\end{frame}
...
\begin{frame}{Formalización: objetivo de extracción de caminos (I--II)}
  \footnotesize
  ...
  \begin{itemize}
    \item $\theta$: parámetros del modelo que propone caminos de relaciones
      $w = (r_1, \dots, r_l)$; $W^{*}$: caminos más cortos entre la entidad
      tema y la respuesta correcta --- la supervisión débil del problema.
    \item Asumiendo $w$ uniforme en $W^{*}$, la Ec.~4 se aproxima promediando
      el log-verosímil sobre $W^{*}$ (Ec.~5, versión computable de la Ec.~4):
  \end{itemize}
  ...
  \begin{itemize}
    \item Un camino es una secuencia, así que su probabilidad se factoriza
      (Ec.~6): entrenar el modelo equivale a predecir, relación por relación,
      la siguiente más plausible dada la pregunta.
    \item El artículo resuelve esto \textbf{entrenando} un LLM específico de KG
      (LLM\_RoG). Esta implementación logra el mismo efecto sin entrenamiento
      (ver sección de desviaciones).
  \end{itemize}
\end{frame}
...
\end{lstlisting}
  ...
  \begin{itemize}
    ...
    \item Modo \texttt{--offline} (\textbf{referencia reproducible}):
      \texttt{StaticLLMClient} con respuestas fijas $\Rightarrow$ siempre inserta
      2 tripletas hacia \emph{United Kingdom}.
    \item Ejecución real contra \texttt{gpt-4o-mini} (temperatura 0.7, sin
      \texttt{--offline}): sin respuestas fijas, cada corrida puede generar
      candidatos distintos --- ver siguiente diapositiva.
  \end{itemize}
\end{frame}
...
\end{lstlisting}
  \small
  \begin{itemize}
    \item Modelo real (\texttt{gpt-4o-mini}, no determinista): sobrevivió la
      coincidencia difusa \emph{Florida}, vía coincidencia accidental con
      \texttt{(Miami, locatedIn, Florida)}, sin relación con la película.
    \item \texttt{(filmPlace, starring)} es \textbf{anclable pero
    ...
    \item Con solo \textbf{2 tripletas insertadas} sobre 13 originales, se
      crean dos rutas hacia \emph{Florida} sin ninguna señal estructural que
      las distinga de la ruta legítima hacia \emph{United States} --- confirma
      el mecanismo central del artículo, aun con un camino semánticamente
      incoherente.
    \item Límite honesto: sin semilla fija, esta corrida no es reproducible
      byte a byte.
  \end{itemize}
\end{frame}
...
\begin{frame}{Desviaciones y limitaciones de alcance}
  \small
  \begin{block}{Extracción de caminos sin modelo entrenado}
    ...
    tema --- sin GPU ni entrenamiento. El resto (respuestas adversarias por
    coincidencia difusa, inserción con estrategia de respaldo) sigue el
  ...
  \end{itemize}
  \vspace{0.5em}
  \textbf{Trabajo futuro:} integrar un recuperador y generador mínimos para
  medir el efecto sobre la respuesta final; ejecutar sobre un subconjunto real
  de WebQSP/CWQ.
\end{frame}

\begin{frame}{Modelos de lenguaje evaluados en el artículo}
  \footnotesize
  \centering
  \begin{tabular}{llccc}
    \toprule
    \textbf{Categoría} & \textbf{LLM} & \textbf{F1 limpio} & \textbf{F1 atacado} & \textbf{Degradación} \\
    \midrule
    Open-source     & Llama-2-7B-chat-hf    & 41.00 & 22.78 & $\downarrow$ 44\% \\
    Open-source     & Llama-3.1-8B-Instruct & 54.20 & 32.02 & $\downarrow$ 41\% \\
    Closed-source   & GPT-3.5-turbo         & 56.51 & 36.19 & $\downarrow$ 36\% \\
    Closed-source   & GPT-4                 & 61.33 & 37.31 & $\downarrow$ 39\% \\
    MoE             & DeepSeek-V3-0324      & 57.69 & 33.00 & $\downarrow$ 43\% \\
    KG-specific     & LLM\_RoG (LLaMA-2-7B) & 70.34 & 38.06 & $\downarrow$ 46\% \\
    \bottomrule
  \end{tabular}
  \vspace{0.5em}
  \begin{itemize}
    \small
    \item Tabla 5 del artículo: robustez de RoG en WebQSP variando \textbf{solo}
      el LLM de generación, con recuperación y prompts fijos.
    \item Para las respuestas adversarias del ataque, el artículo usa
      \textbf{GPT-4}; para GCR/SubgraphRAG usa \textbf{GPT-3.5-turbo} como
      backend --- ninguno de estos seis modelos coincide con el usado aquí.
  \end{itemize}
  \vspace{0.3em}
  \begin{alertblock}{Esta implementación}
    \small Usa únicamente \texttt{gpt-4o-mini} en las dos etapas basadas en
    LLM (respuestas adversarias y caminos de relaciones).
  \end{alertblock}
\end{frame}
