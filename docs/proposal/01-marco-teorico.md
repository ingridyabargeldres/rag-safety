...
documentación: qué es un [grafo de conocimiento](#grafo-de-conocimiento), cómo
se representan los [caminos de razonamiento y de relaciones](#caminos-de-razonamiento-y-caminos-de-relaciones)
que lo recorren, qué significa formalmente [responder una pregunta sobre ese
grafo](#preguntas-sobre-grafos-de-conocimiento) y qué tarea resuelve, en la
práctica, un [sistema KG-RAG](#pipeline-de-un-sistema-kg-rag). Sobre esa base
se formaliza el [modelo de amenaza del atacante](#objetivo-del-atacante) y las
dos piezas matemáticas centrales del ataque: cómo se generan [caminos de
relaciones plausibles](#formulación-de-la-extracción-de-caminos-de-relaciones)
y cómo se construye, a partir de ellos, la [tripleta de
perturbación](#anclaje-del-prefijo-y-construcción-de-la-tripleta-de-perturbación).
Las definiciones y fórmulas de este documento siguen a Zhao et al. (2025).

## Grafo de conocimiento

Un grafo de conocimiento se define como una terna $`G = (E, R, T)`$, donde $`E`$ es el
conjunto de entidades, $`R`$ el conjunto de relaciones y $`T \subseteq E \times R \times E`$ el conjunto
de tripletas fácticas. Cada tripleta $`(e_h, r, e_t) \in T`$ representa una arista
dirigida y etiquetada: la relación $`r`$ conecta la entidad cabeza $`e_h`$ con la
entidad cola $`e_t`$. Por ejemplo, $`(\text{Manchester}, \text{locatedIn}, \text{Massachusetts})`$ afirma
...
del camino, descartando las entidades intermedias: $`w = (r_1, r_2, \ldots, r_l)`$. Por
ejemplo, el camino de razonamiento $`\text{Cardiff} \xrightarrow{\text{locatedIn}} \text{Wales} \xrightarrow{\text{containedIn}} \text{United Kingdom}`$
tiene como camino de relaciones asociado $`(\text{locatedIn}, \text{containedIn})`$.
...
prompt y se pasa a un modelo de lenguaje parametrizado por $`\theta`$, que produce la
respuesta final:
...
entidad tema de la pregunta —la entidad principal que esta menciona—, la etapa
de recuperación puede incluirlas dentro de
...
adversarias $`\hat{T}`$ en el grafo, de modo que el sistema KG-RAG, operando sobre el
grafo modificado $`\hat{G} = \{E, R, T \cup \hat{T}\}`$, produzca un conjunto de respuestas $`\hat{A}`$ que
...
existentes—. Para mantener el ataque difícil de detectar, cada tripleta insertada
...
Dado un [camino de relaciones](#caminos-de-razonamiento-y-caminos-de-relaciones) $`w`$, su prefijo $`w' = (r_1, \ldots, r_{l-1})`$ se ancla en el grafo a partir de la entidad tema $`e_q`$ de la pregunta,
...
donde cada $`e_{l-1} \in E_{l-1}`$ es una entidad real, alcanzable siguiendo
$`e_q \xrightarrow{r_1} e_1 \xrightarrow{r_2} \cdots \xrightarrow{r_{l-1}} e_{l-1}`$. La tripleta de perturbación se completa
adjuntando la última relación del camino y la respuesta adversaria: $`(e_{l-1}, r_l, \hat{a})`$. La intuición es que, si un sistema KG-RAG ya seguiría el prefijo $`w'`$ al
razonar sobre la pregunta, basta con una única arista adicional —$`r_l`$ hacia $`\hat{a}`$—
para que el camino de razonamiento termine en la respuesta incorrecta en lugar de
la correcta.
...
camino de dos saltos $`(r_1, r_2)`$, se insertan juntas las tripletas $`(e_q, r_1, e')`$ y
$`(e', r_2, \hat{a})`$, donde $`e'`$ es una entidad puente elegida al azar.
